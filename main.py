...
state_estimate = np.array([6.7e6, 0, 0, 7e3])
...
while t < 60000:
    ...
    measured_state = np.array([xm, ym, vxm, vym])

    # ----- FILTER NOISE -----
    state_estimate = filter.filter(measured_state, state_estimate, 
                                   controller, mu)
...
settling_time = all_times[last_outside]
